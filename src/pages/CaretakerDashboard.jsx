...
const StatCard = ({ label, number, help, type, pct, color, icon }) => (
  <Box p={6} bg="white" rounded="3xl" shadow="sm" border="1px solid" borderColor="gray.100">
    <Flex justify="space-between" align="start">
      <Box>
        <Text fontSize="xs" fontWeight="bold" color="gray.400" textTransform="uppercase" mb={2}>{label}</Text>
        <Heading size="lg" fontWeight="900" color="gray.800" mb={1}>{number}</Heading>
        <HStack spacing={1}>
          <StatArrow type={type} />
          <Text fontSize="xs" fontWeight="bold" color={type === "increase" ? "green.500" : "red.500"}>{pct}</Text>
          <Text fontSize="xs" color="gray.400" ml={1}>{help}</Text>
        </HStack>
      </Box>
